...
unit_stop_ordering() {
  ...
  ' _ "$st" "$marker" &
  # shellcheck disable=SC2031 # Captures this function's background daemon PID.
  daemon_pid=$!
  lock="$st/state/.supervise-daemon.lock"
  mkdir -p "$lock"
...
unit_stop_rejects_reused_pid() {
  ...
  date '+%s' > "$st/state/.afk"
  sleep 600 &
  # shellcheck disable=SC2031 # Captures this function's background sleeper PID.
  sleeper_pid=$!
  lock="$st/state/.supervise-daemon.lock"
  mkdir -p "$lock"
...
unit_concurrent_start_serialized() {
  ...
  # shellcheck disable=SC2031 # Captures this function's first concurrent launch PID.
  first=$!
  FM_HOME="$st" FM_STATE_OVERRIDE="$st/state" FM_SUPERVISOR_TARGET="$cap_pane" \
    FM_SUPERVISOR_BACKEND=tmux FM_AFK_LAUNCH_ENTRY="$SLEEPER" "$LAUNCH" start >/dev/null 2>&1 &
  # shellcheck disable=SC2031 # Captures this function's second concurrent launch PID.
  second=$!
...
unit_lock_initialization_grace() {
  ...
  # shellcheck disable=SC2031 # Captures this function's background initializer PID.
  initializer=$!
...
unit_signal_exits_with_lock_cleanup() {
  ...
  FM_HOME="$st" FM_STATE_OVERRIDE="$st/state" bash -c '
    ...
    : > "$2"
  ' _ "$LAUNCH" "$marker" &
  # shellcheck disable=SC2031 # Captures this function's background child PID.
  child=$!
