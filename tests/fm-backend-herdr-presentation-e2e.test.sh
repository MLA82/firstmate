...
case "$ABORT_SEQUENCE" in
  $'create-a\nclose-a\ncreate-b\nclose-b'|$'create-b\nclose-b\ncreate-a\nclose-a') ;;
  *)
    printf 'DIAG focus-audit since abort start:\n' >&2
    sed -n "$((ABORT_FOCUS_START + 1)),\$p" "$FOCUS_AUDIT_LOG" | cut -c1-200 >&2
    printf 'DIAG herdr calls since abort start:\n' >&2
    sed -n "$((ABORT_START + 1)),\$p" "$HERDR_CALL_LOG" | cut -c1-200 | tail -150 >&2
    printf 'DIAG treehouse calls:\n' >&2
    tail -30 "$TREEHOUSE_CALL_LOG" >&2
    printf 'DIAG abort-a err:\n' >&2; cat "$TMP_ROOT/abort-a.err" >&2
    printf 'DIAG abort-b err:\n' >&2; cat "$TMP_ROOT/abort-b.err" >&2
