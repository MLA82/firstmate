...
hold_task_set_lock() {  # <state-dir> -> echoes "<holder-pid> <lock-path>"
  ...
  # shellcheck disable=SC2031 # Captures this function's background lock-holder PID.
  holder=$!
...
SH
  ...
  PATH="$fakebin:$PATH" FM_HOME="$home" FM_FAKE_TMUX_LOG="$log" \
    ...
    "$ROOT/bin/fm-teardown.sh" domain --force >/dev/null 2>"$err" &
  # shellcheck disable=SC2031 # Captures this function's background teardown PID.
  pid=$!
...
EOF
  ...
  # shellcheck disable=SC2031 # Captures this function's background watcher PID.
  pid=$!
