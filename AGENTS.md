...
Run `bin/fm-pr-check.sh <id> <PR url>` with the exact URL from the worker's ready signal when that signal names one; the script header owns its fail-closed registration checks, after which it records `pr=` and the forge's `pr_head=` when available and arms the watcher's merge poll.
...
Whenever a PR is mentioned, include its full `https://...` URL before any shorthand reference, copied from the worker's own report or the recorded PR rather than composed from memory.
