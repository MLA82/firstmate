...
An acknowledgement whose cutoff removes none of the actor's rows while a presented row above the cutoff still waits is reported as having acknowledged nothing and names that presented row.
While its recovery generation remains active, the result includes the exact `--ack-through` and `--recovery-generation` command for that row; if the episode is already retired, it directs a fresh drain to mint and print the replacement command.
...
`tests/fm-wake-queue.test.sh`'s mixed-queue actor, stale-acknowledgement, and presentation-deadline tests drive the real scripts: branch acknowledgement cannot swallow a main row, a stale acknowledgement names the current presented row and usable remedy, a concurrent main turn cannot present or acknowledge an active branch grant, live-holder presentation contention stays bounded and retriable, and acknowledgement locking remains blocking.
