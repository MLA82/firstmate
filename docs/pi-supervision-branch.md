...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  `signal_files_actionable` marks that signal row's queued payload `needs-decision:` instead of `signal:`, `scopeForUnreadWake` excludes the marked row from what the branch may claim, and the dispatcher forces that signal close to main by cross-referencing the trigger's file list against the excluded keys.
  ...
  The identical non-veto guarantee applies to a co-present marked needs-decision signal row.
