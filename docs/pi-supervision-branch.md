...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  While a prompt is open, `fm_branch_report` accepts only `fleet` and the tasks that prompt's claimed rows resolve to (a signal row by its status-log key, a stale row through the task record naming that endpoint), or for a heartbeat review any task with a live record at prompt time.
  Each allowed task is bound to the `spawn_gen` captured with that prompt, and the outcome append verifies the same live generation under its store lock, so a task remembered from another wake, retired after the prompt opened, or replaced under the same id is refused before anything is stored or delivered.
  While the branch retains verified fleet-lock ownership, its guarded commands never tell it to drain queued rows mid-handling: `bin/fm-guard.sh` names the granted rows it is handling instead.
  An acknowledgement that consumes nothing names the current presented row and, while its recovery generation remains active, the exact command that acknowledges it; [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns that contract.
...
`fleet` is a protocol sentinel rather than a worker identity, so `fm-spawn.sh` reserves it from every task spawn.
...
`tests/fm-wake-drain-outcome-backstop.test.sh` covers keyless resurfacing, causal suppression, same-second ordering, one-shot presentation, first-drain index self-healing under the outcome lock without resurrecting retired-task indexes, store-fault fail-closed behavior, bounded history cost and output, and the oversized-line limit; `tests/fm-teardown.test.sh` covers retiring the task's index.
The branch-offer, heartbeat-offer, heartbeat-not-ridden-by-a-check, and main-only-check-class tests remain in `tests/fm-pi-watch-extension.test.sh`, the recovery test remains in `tests/fm-session-start.test.sh`, and `tests/fm-wake-queue.test.sh` covers per-actor consumption plus stale acknowledgements that name the current row and remedy.
