...
`bin/fm-pr-check.sh`'s header owns the fail-closed registration contract for canonical spelling, live forge resolution, conditional worker-status provenance, and bounded lookup behavior.
`bin/fm-pr-merge.sh` also rejects malformed URLs or repository override flags before recording merge state.
