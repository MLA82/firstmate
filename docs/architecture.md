...
Tracked-file dirty clones, non-default branches, detached HEADs with unique commits, diverged defaults, and default branches checked out in another worktree are reported as `STUCK:` with their behind count and left untouched, as is a clone whose re-attach checkout Git refuses (for example over a colliding untracked file), with Git's reason appended; a fast-forward Git refuses is instead a quiet skip carrying Git's reason, the same as any other unmet fast-forward precondition.
