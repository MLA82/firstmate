...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled or if work, process-event sources, or Relay polling has an unhealthy model-aware supervision verdict.
For a mutable main actor it also warns when queued wakes are waiting to be drained; [`pi-supervision-branch.md`](pi-supervision-branch.md#components-and-their-owners) owns the branch actor's grant-aware mid-handling guidance.
