...
- `FLEET_SYNC: <repo>: skipped: <reason>` - a benign one-off skip (offline, no origin, local-only), or a fast-forward genuinely blocked by a colliding untracked file (`fast-forward failed: ...`); bootstrap continued, investigate only if it blocks work.
...
- `FLEET_SYNC: <repo>: STUCK: on <state>, N commits behind <base> - needs attention` - the clone has tracked-file changes (untracked-only never counts), is on a non-default branch, is detached with unique commits, or has diverged, so the sync left it untouched (never forcing or discarding); it will keep falling behind until you look.
  A detached HEAD whose re-attach checkout git refused (for example over a colliding untracked file) is STUCK too, with `(checkout failed: <git's reason>)` appended.
