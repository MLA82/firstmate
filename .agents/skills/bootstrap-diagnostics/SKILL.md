...
- `FLEET_SYNC: <repo>: STUCK: on <state>, N commits behind <base> - needs attention` - the clone has tracked-file changes (untracked-only never counts), is on a non-default branch, is detached with unique commits, or has diverged, so the sync left it untouched (never forcing or discarding); it will keep falling behind until you look.
  A re-attach checkout that git refused (for example over a colliding untracked file) is STUCK too, with `(checkout failed: <git's reason>)` appended. A fast-forward that git refuses - whatever the reason, including a colliding untracked file - is instead a quiet `skipped: fast-forward failed: <git's reason>`, the same as any other unmet fast-forward precondition.
