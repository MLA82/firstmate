...
- `FLEET_SYNC: <repo>: skipped: <reason>` - a benign one-off skip (offline, no origin, local-only), or a fast-forward/checkout genuinely blocked by a colliding untracked file (`fast-forward failed: ...` / `checkout failed: ...`); bootstrap continued, investigate only if it blocks work.
...
- `FLEET_SYNC: <repo>: recovered: <detail>` - the clone had drifted onto a detached HEAD with no tracked-file changes holding no unique commits and the sync self-healed it (re-attached the default branch and fast-forwarded); no action needed, it is reported only so the self-heal is visible.
  `<detail>` may add `(untracked files present)` - informational, neither blocks or needs immediate action.
